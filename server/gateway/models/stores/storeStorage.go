...
func (ss *StoreStorage) GetStores(userId int64) ([]*Store, error) {
	rows, err := ss.DB.Query("select * from stores where id=?", id)
	if err != nil {
		return nil, fmt.Errorf("%v: %v", ErrStoreNotFound, err)
	}
...
func (ss *StoreStorage) Insert(store *Store) (*Store, error) {
	insq := "insert into stores(store_id, user_id, store_name, price) values (?,?,?,?)"

	res, err := ss.DB.Exec(insq, store.ID, store.UserID, store.StoreName, store.StoreLocation)
	if err != nil {
